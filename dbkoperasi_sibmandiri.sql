...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 20, 2025 at 08:14 AM
...
SELECT stok INTO produk_stok
FROM produk WHERE id = produk_id;
IF produk_stok >= jumlah_pesanan THEN
UPDATE produk SET stok = stok - jumlah_pesanan
WHERE id = produk_id;
END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `pro_naikan_harga` (IN `jenis_produk` INT, IN `persentase_kenaikan` INT)   BEGIN
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `kategori_harga` (`harga` DOUBLE) RETURNS VARCHAR(15) CHARSET utf8mb4 COLLATE utf8mb4_general_ci DETERMINISTIC BEGIN
...
(4, 'NO', 'Non Member', 0, 0),
(5, 'KRT180', 'Gold Level 2', 2, 200000),
(6, 'KRT909', 'Platinum Lvl 2', 1, 100000);

--
-- Triggers `kartu`
--
DELIMITER $$
CREATE TRIGGER `isi_kode_kartu` BEFORE INSERT ON `kartu` FOR EACH ROW BEGIN
IF NEW.kode IS NULL OR NEW.kode = '' THEN
SET NEW.kode = CONCAT('KRT', LPAD(FLOOR(RAND() * 1000), 3, '0'));
END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `isi_kode_kartu_update` BEFORE UPDATE ON `kartu` FOR EACH ROW BEGIN
IF NEW.kode IS NULL OR NEW.kode = '' THEN
SET NEW.kode = CONCAT('KRT', LPAD(FLOOR(RAND() * 1000), 3, '0'));
END IF;
END
...
(4, 'KW001', '2023-03-03', 17500, 1, 2, 'Lunas'),
(5, 'KW002', '2025-03-05', 200000, 1, 1, 'Belum Lunas');
...
SELECT IFNULL(SUM(jumlah), 0) INTO total_bayar FROM pembayaran
...
--
DELIMITER $$
CREATE TRIGGER `kurangiStok` AFTER INSERT ON `pesanan_items` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `normalisasiStok` AFTER DELETE ON `pesanan_items` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `trig_kurangi_stok` AFTER INSERT ON `pesanan_items` FOR EACH ROW BEGIN
CALL kurangi_stok(NEW.produk_id, NEW.qty);
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `updateStok` AFTER UPDATE ON `pesanan_items` FOR EACH ROW BEGIN
...
(1, 'TV01', 'Televisi 21 inch', 3500000, 5451264, 22, 2, 1),
...
(5, 'TK01', 'Teh Kotak', 3000, 3500, 0, 10, 4),
(6, 'PC01', 'PC Desktop HP', 7000000, 10560000, 9, 2, 5),
(7, 'TB01', 'Teh Botol', 2000, 2500, 53, 10, 4),
(8, 'AC01', 'Notebook Acer', 8000000, 11880000, 7, 2, 5),
(9, 'LN01', 'Notebook Lenovo', 9000000, 13200000, 9, 2, 5),
(10, 'L004', 'Laptop HP', 12000000, 14300000, 20, 5, 5),
...
(14, 'SM001', 'Susu Murni', 15000, 20000, 10, 5, 3);
